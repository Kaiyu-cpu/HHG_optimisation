...
def fitness_func(delay_12,peak_intensity_1, t=t, x=x, low_boundary=low_boundary, plot=False,Print=False,title='',show_driving=False):
    print('para:', delay_12,peak_intensity_1)
    peak_intensity_2=3.12-peak_intensity_1
    #convert to SI unit
    peak_intensity_1 = peak_intensity_1*1e18
    peak_intensity_2 = peak_intensity_2*1e18
    ...
    # Three step fitting
    q1=250
    initial_guesses = [q1,q1+30,-0.1,-65,-85]
    ...
    if abs(m1)<abs(m2) and m1<0 and m2<0 and q1>0 and q2<400:
        ...
        search_space=[50,100,150,200,300,350]
        for i in search_space:
            initial_guesses = [i,i+30,-0.1,-65,-85]
            fit, fit_cov = curve_fit(three_step,x_new,y_new,p0=initial_guesses)
            q1,q2,m1,c1,c3 = fit
            m2 = (c3-m1*q1-c1)/(q2-q1)
            if abs(m1)<abs(m2) and m1<0 and m2<0 and q1>0 and q2<400:
...
pbounds = {'delay_12':(-48,48),'peak_intensity_1':(0,3)} # delay in fs
